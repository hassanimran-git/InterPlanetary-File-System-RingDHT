...
public:
	...
	RingDHT(int bits, int m, bool opt)
	{
		...
		tree_order = m;
		manual = opt;
		unique_chars = calculateUniqueChars(stoi(iSpace));
	}
	void UpdateRouting()
	{
		Machine *ptr = head;
	...
	void InsertMac(string id, string macName)
	{
		...
		if (traversal_Ptr->next->btree->root == NULL)
		{ // successor is empty
			return;
		}

		Machine *successor = traversal_Ptr->next;

		// adjusting the btrees to snatch files from the successor
		while (1)
		{
			Info temp = successor->btree->smallest();
			if (temp.key <= traversal_Ptr->M_Id)
			{
				traversal_Ptr->btree->insert(new Info(temp));
